...
export const sortEvent = (allEvents, filteredProjects, todos) => {
  ...
  // - 完成期限早於等於今天者，對resource push進「1」(排序最前、背景紅色字白色)
  // - 完成期限為明天者，對resource push進「3」(排序第二、背景橘色字白色)
  // - 完成期限為後天以後者，對resource push進「5」(排序第三、背景正常字正常)
  const addPriority = [];
  const addPriorityLogic = allEvents.map((event) => {
    ...
    switch (true) {
      case deadline.diff(today, "days") <= 0:
        event.resource.push("1");
        break;
      case deadline.diff(today, "days") === 1:
